...
class freak_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'freak_rp', // Base ID
			__('Freak Recent Posts with Thumbnails', 'freak'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'freak' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         ?>
		         <div class='rp-thumb'><a href="<?php the_permalink(); ?>"><img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/placeholder2.jpg"></a></div>
		         <?php
		         endif; ?>	
		...
		?>
		     ...
		     <?php esc_html_e( 'Oops, there are no posts.','freak'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'freak' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'freak' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:','freak' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php esc_html_e( 'No. of Posts:', 'freak' ); ?></label>
...
add_action( 'widgets_init', 'register_freak_widget' );  
function register_freak_widget() {  
    register_widget( 'freak_Recent_Posts' );  
}  
